  ...
  case MODE_TETRIS:
    ...
    if (total_lines >= 100) {
      ...
      sf2d_draw_texture(misc_num[currentnum % 10],
                        ...
                        lines_frame.posy + digit_offset_linesy);
      currentnum = currentnum / 10;
      // first digit (actually this way it won't crash after getting >999, just
      // won't display the number of thousands)
      sf2d_draw_texture(misc_num[currentnum % 10],
                        lines_frame.posx + (lines_frame.texture->width >> 1) -
      ...
      sf2d_draw_texture(misc_num[total_lines],
                        ...
                        lines_frame.posy + digit_offset_linesy);
    if (!paused && !gameover) {
      if (controllable && !ARE_state) { // don't render the block while we're
                                        // clearing lines nor during ARE.
        if (cfg.ghost_piece)
      ...
      case TETRIS:
        if (tetris_indicator)
          sf2d_draw_texture(tetris_indicator, indicatorx + offset, indicatory);
      ...
      case TSPIN:
        if (tspin_indicator)
          sf2d_draw_texture(tspin_indicator, indicatorx + offset, indicatory);
        indicator_frames++;
        break;
      case TSPINSINGLE:
