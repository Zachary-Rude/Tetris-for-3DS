...
u8 audio_init(const char *template) {
  ...
  music.ndsp_format =
      vi->channels == 1 ? NDSP_FORMAT_MONO_PCM16 : NDSP_FORMAT_STEREO_PCM16;
  ...
  aptOpenSession(); // make the ogg input/decoder thread run on second core -
                    // well, actually not really - second core's too slow
...
void audio_music_load() {
  ...
  while (runThread) {
    ...
    // past this point - means we need to swap the buffers, and the first is
    // finished/used
    ...
    looped_vorbis_read(music.file, music.second_data,
                       music.total_buffer_size); // load the actual data
...
void audio_music_check() {
  ...
  if (music.last_check > cur_sample &&
      !ndspChnIsPaused(
          music.chnl)) // means it went to the second buffer - actually a bad
                       // idea for small buffer sizes
  {
...
void looped_fread(u32 bytes_to_read, u32 start_point, u32 data_size, FILE *fp,
                  u8 *buffer) {
...
void looped_vorbis_read(OggVorbis_File *vf, u8 *buffer, u32 length) {
  ...
  while (total_read != length) {
    bytes_read = ov_read(vf, (char *)buffer + total_read, length - total_read,
